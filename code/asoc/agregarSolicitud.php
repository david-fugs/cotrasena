...
// Configurar límites para manejo de archivos
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($mysqli->query($query)) {
    ...
    }    // Información básica sobre límites de PHP (solo log crítico)
    ...
    if (isset($_FILES['archivos']) && is_array($_FILES['archivos']['error'])) {
        ...
        if (isset($_FILES['archivos']) && $_FILES['archivos']['error'][0] === UPLOAD_ERR_NO_FILE) {
            // Usuario no seleccionó archivos - comportamiento normal
        } else {
    ...
    if ($hayArchivos) {
      foreach ($_FILES['archivos']['name'] as $index => $fileName) {
        ...
        $tmpName = $_FILES['archivos']['tmp_name'][$index];
          ...
          if (move_uploaded_file($tmpName, $destination)) {
              ...
              if (!file_exists($destination)) {
                  $erroresArchivos[] = "Archivo $fileName: Error en la verificación final";
              }
          } else {
        ...
        // Usuario no subió archivos, continuando sin archivos
    }
